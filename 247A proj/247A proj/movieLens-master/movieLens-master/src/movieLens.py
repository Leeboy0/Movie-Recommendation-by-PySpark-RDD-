...
def recommend_for_all(model, movies, result_path, file_out_flag=False):
    ...
    recommendation_result = products_for_all_users.repartition(1).orderBy(products_for_all_users.user).rdd \
        ...
        .toDF().repartition(1)
    if file_out_flag:  # output to file
...
def main(spark, data_path, result_path):
    ...
    # Whether to use the default parameters or not, select the optimal parameter from a series of parameters when it is False.
    default_flag = True
    if default_flag:
        # Build the model using default parameters
        model = train_model(training)
    else:
        # Select the optimal parameters to establish the model
        training.cache()
        model = tuning_model(training, test)
        training.unpersist()

    # Make recommendations for all the following users and output them to the terminal
    user_ids = list(u.userId for u in all_users.take(3))
    print "userIds: ", user_ids
    recommend_for_users(model, movies, user_ids)

    # Make recommendations for all users and output them to a file
    file_out_flag = True
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        data_path = sys.argv[1]  # Data directory
        result_path = sys.argv[2]  # Save the result directory
    else:
